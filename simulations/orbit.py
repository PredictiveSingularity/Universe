...
def input(key):
    ...
    if key == 'u':
        paused = False
    # if key == '1':
    #     human.position = (mercury.x - 20, mercury.y, mercury.z - 20)
    # if key == '2':
    #     human.position = (venus.x - 20, venus.y, venus.z - 30)
    # if key == '3':
    #     human.position = (earth.x - 40, earth.y, earth.z - 20)
    # if key == '4':
    #     human.position = (mars.x - 50, mars.y, mars.z - 50)
    # if key == '5':
    #     human.position = (jupiter.x - 200, jupiter.y, jupiter.z - 200)
    # if key == '6':
    #     human.position = (saturn.x - 200, saturn.y, saturn.z - 200)
    # if key == '7':
    #     human.position = (uranus.x - 200, uranus.y, uranus.z - 200)
    # if key == '8':
    #     human.position = (neptune.x - 200, neptune.y, neptune.z - 200)
...
class Human(FirstPersonController):
    human_cursor = Entity(parent=camera.ui, model='quad', color=color.pink, scale=.008, rotation_z=45)
    human_speed = 0
    human_height = 2
    mouse_sensitivity = Vec2(50, 50)
    gravity = 1

    def __init__(self, x, y, z, name, human_cursor=human_cursor, human_speed=human_speed, human_height=human_height, mouse_sensitivity=mouse_sensitivity, gravity=gravity):
        super().__init__(human_cursor=human_cursor, human_speed=human_speed, human_height=human_height, mouse_sensitivity=mouse_sensitivity, gravity=gravity)
...
human = Human(-250+800, 8*10+1, 8*10+1, "Player")
